...
class _DocumentListScreenState extends ConsumerState<DocumentListScreen> {
          ...
          Padding(
            ...
            child: Row(children: [
              Expanded(child: TextField(
                controller: _search,
                decoration: InputDecoration(labelText: l10n.timelineSearchLabel, prefixIcon: const Icon(Icons.search_rounded)),
                onSubmitted: (_) => _reload(subjectId),
              )),
              const SizedBox(width: AppSpacing.sm),
              IconButton(icon: const Icon(Icons.filter_list_rounded), tooltip: l10n.timelineFiltersAction, onPressed: () => _chooseStatus(subjectId)),
            ]),
          ),
  ...
  Future<DocumentPage> _load(String? subjectId) => ref.read(documentRepositoryProvider).listDocuments(
    subjectId: subjectId, status: _status, query: _search.text,
  );
  ...
  Future<void> _chooseStatus(String? subjectId) async {
    final value = await showModalBottomSheet<DocumentStatus?>(
      ...
      builder: (context) => ListView(shrinkWrap: true, children: [
        ListTile(title: Text(context.l10n.timelineAllTypes), onTap: () => Navigator.pop(context)),
        for (final status in DocumentStatus.values)
          ListTile(title: Text(status.apiName), onTap: () => Navigator.pop(context, status)),
      ]),
    );
    if (!mounted) return;
    setState(() { _status = value; _page = _load(subjectId); });
  }
}
