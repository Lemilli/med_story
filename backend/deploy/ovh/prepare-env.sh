...
sed \
  ...
  -e "s|<strong-admin-db-password>|${database_admin_password}|g" \
  -e "s|<strong-migrator-db-password>|${database_migrator_password}|g" \
  -e "s|<strong-app-db-password>|${database_app_password}|g" \
  -e "s|<default-from-email>|${from_email}|" \
  "${backend_dir}/.env.production.example" > "${environment_file}"
chmod 600 "${environment_file}"

echo "Created ${environment_file} with generated Django and separate PostgreSQL role secrets."
