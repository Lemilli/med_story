...
| OpenAI | Core LLM, OCR, and STT processing through backend workers | Transient document/audio content and extracted text; minimum necessary health data | Any `AI_*_PROVIDER=openai` setting | Self-review updated 2026-08-06 for mandatory processing disclosed in the required privacy notice; no training opt-in, `store=false`, default abuse monitoring accepted, no ZDR claim | `processor-dpa-review.md`; account/project evidence to be retained privately | MedStory operator; 2027-08-06 or material service change |
